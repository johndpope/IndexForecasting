...
port_sharpe = sum(daily_ret(:,best_sharpe)+1,2)/4;
port_ret    = (geomean(port_sharpe)^trading_days)-1;
port_std    = std(port_sharpe) * sqrt(trading_days);
port_sharpe = (port_ret - rf)/port_std;

sp_ret      = (geomean((daily_ret(:,sp500)+1))^trading_days)-1;
sp_std      = std(daily_ret(:,sp500)) * sqrt(trading_days);
sp_sharpe   = (sp_ret-rf)/sp_std;


figure
subplot(1,2,1)
plot(cumprod(mean(daily_ret(:,best_sharpe),2)+1))
title('Highest Sharpe Ratio Cum-Returns')
xlabel('Time (days)')
ylabel('Standardised Returns')
subplot(1,2,2)
plot(cumprod(daily_ret(:,sp500)+1))
title('SP500 Cum-Returns')
xlabel('Time (days)')
ylabel('Standardised Returns')
